...
function generateTuples(config) {
  ...
  // 5. Group-based assignments per document (200–500 group#member tuples each)
  console.log('  Generating group-based assignments per document...');
  for (let d = 0; d < config.documents; d++) {
    const count = 200 + Math.floor(Math.random() * 301); // [200, 500]
    for (let i = 0; i < count; i++) {
      const groupId = (d * count + i) % config.groups;
      const relation = RELATIONS[i % RELATIONS.length];
      tuples.push({
  ...
  // 6. Extra group-based assignments to reach targetTuples (if still short)
  console.log('  Generating extra group-based assignments...');
